...
class SessionManager 
{
  ...
  public function set(Player $player, bool $firstJoin = false): void
  {
    ...
    Loader::getInstance()->getDatabase()->executeImplRaw([0 => "SELECT * FROM duration,murders,deaths,won_events,user_data,settings WHERE duration.xuid = '$xuid' AND murders.xuid = '$xuid' AND deaths.xuid = '$xuid' AND won_events.xuid = '$xuid' AND user_data.xuid = '$xuid' AND settings.xuid = '$xuid'"], [0 => []], [0 => SqlThread::MODE_SELECT], function(array $rows) use($session, $player) {
